...
def fitGaus(roi):
    ...
    linsize = int(np.sqrt(npx2-2))

    arx = float(ar%linsize)
    ary = float(ar//linsize)
    #      Amplitude, sx, sy, bg, x, y
    #seed2 = (mmax-mmin,1.5,1.5,mmin,arx,ary)                 
    x =np.array([np.arange(linsize) for j in range(linsize)])
    y =  np.transpose(x).flatten()
    x = x.flatten()
    ...
    try:
        #print("Hello")
        print(linsize,npx2)
        popt, pcov = curve_fit(twoD_GaussianA, (x, y), roi[:(npx2-2)], p0=(np.log(mmax-mmin),1.5,1.5,0.1,np.log(mmin),arx,ary))
        #print("Hello2")
...
def fitGausfxth(roi):
    ...
    try:
        #print("Hello")
        popt, pcov = curve_fit(twoD_Gaussianfxth, (x, y), roi[:(npx2-2)], p0=(np.log(mmax-mmin),1.5,1.5,np.log(mmin),arx,ary))
        #print("Hello2")
        popt = np.concatenate((popt,[0]))
    except Exception as e: 
        #print(e)
...
def fitGausfxthpi4(roi):
    ...
    try:
        #print("Hello")
        popt, pcov = curve_fit(twoD_Gaussianfxthpi4, (x, y), roi[:(npx2-2)], p0=(np.log(mmax-mmin),1.5,1.5,np.log(mmin),arx,ary))
        #print("Hello2")
        popt = np.concatenate((popt,[0]))
    except Exception as e: 
        #print(e)
...
def fitGausfxsh(roi):
    ...
    s1, s2 = (roi[npx4-2],roi[npx4-1])
    ar = int(roi[npx4-4])
    mmax = roi[ar]
    mmin = roi[int(roi[npx4-3])]

    linsize = int(np.sqrt(npx4-4))
    ...
    try:
        ...
        popt, pcov = curve_fit(gausfixsh, (x, y), roi[:(npx4-4)], p0=(np.log(mmax-mmin),np.log(mmin),arx,ary))
        #print("Hello2")
...
def fitGausJac(roi):
    ...
    try:
        #print("Hello")
        popt, pcov = curve_fit(twoD_GaussianA, (x, y), roi[:(npx2-2)], p0=(np.log(mmax-mmin),1.5,1.5,0.1,np.log(mmin),arx,ary),jac=TwodGJacF)
        #print("Hello2")
        popt = np.concatenate((popt,[0]))
    except Exception as e: 
        #print(e)
