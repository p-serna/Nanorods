...
def centroid2(data):
    ...
    x1=ones((1,h))
    y1=ones((w,1))
    return ((dot(dot(x1, data), y))/(dot(dot(x1, data), y1)),
            (dot(dot(x, data), y1))/(dot(dot(x1, data), y1)))
...
for dirt in dirs:
    ...
    dfiles = []
    for f in files:
        if f[-4:]=='.tif': dfiles.append(basedir+f)
...
for f in dfiles:
    ...
    par0 = array([log(cf1),cf0,log(max(totalFA[1]-cf1,1e-5)),3000.0])
